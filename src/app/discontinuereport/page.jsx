...
const Page = () => {
  ...
  const fetchOptions = (settlements) => {
    const uniqueSchemeTypes = [
      ...new Set(settlements.map((settlement) => settlement.SchemeType)),
    ];
    const uniqueSchemeNames = [
      ...new Set(settlements.map((settlement) => settlement.SchemeName)),
    ];
  ...
  useEffect(() => {
    ...
    if (schemeType) {
      filteredData = filteredData.filter(
        (settlement) => settlement.SchemeType === schemeType
      );
    }
    ...
    if (schemeName) {
      filteredData = filteredData.filter(
        (settlement) => settlement.SchemeName === schemeName
      );
    }

    // Filter by searchTerm (Card No)
    if (searchTerm) {
      filteredData = filteredData.filter((settlement) =>
        settlement.CardNo.includes(searchTerm)
      );
    }
    ...
    if (searchMember) {
      filteredData = filteredData.filter((settlement) =>
        settlement.MemberName.includes(searchMember)
      );
    }
    ...
    if (searchMobile) {
      filteredData = filteredData.filter((settlement) =>
        settlement.MobileNo.includes(searchMobile)
      );
    }
